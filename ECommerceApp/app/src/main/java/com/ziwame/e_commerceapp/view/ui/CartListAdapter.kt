...
class CartListAdapter(
    ...
    private var gadgetList: List<CartProduct>?,
    ...
    override fun onBindViewHolder(holder: DataViewHolder, position: Int) {
        Glide.with(context).load(gadgetList?.get(position)?.image_url)
            .into(holder.imageView)
        holder.Price.text = gadgetList?.get(position)?.price
        holder.Name.text = gadgetList?.get(position)?.name
